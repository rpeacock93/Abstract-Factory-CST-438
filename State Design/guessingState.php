<?php 
	...
	class guessingState implements GameState {

		public function guessingState($newGameMachine) {
		...
		public function displayHint() {
			// displays calculated hint from compareLocation
		...
		public function drawBoard() {
			// draws board with previous user guess
		...
		public function makeGuess($xGuess, $yGuess) {
			...
			if (($intHurkleX == $intPlayerX) and ($intHurkleY == $intPlayerY)) {
				...
				if (($this->gameMachine->movesLeft) == 0) {
					// moves to losing state
					$this->gameMachine->setGameState($this->gameMachine->loseState());
				} else {
